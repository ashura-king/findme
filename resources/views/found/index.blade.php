...
@section('title', 'Found Items - Lost & Found')
...
@section('content')
<div class="container-fluid found-items-container">
  @if(session('success'))
  ...
  <div class="row mb-4 align-items-center">
    <div class="col-md-6">
      <h1 class="h2 mb-2 fw-bold text-warning">Found Items</h1>
      <div class="item-counter">
        Showing <span id="item-count" class="fw-bold text-warning">{{ $items->count() }}</span> item(s)
      </div>
    </div>
    ...
    <div class="col-md-6">
      <div class="search-container">
        ...
        <input type="text" id="search-input" class="form-control search-input"
          placeholder="Search found items by name, location, or category...">
      </div>
    </div>
  </div>
  ...
  <div id="no-results" class="card shadow-sm d-none">
    <div class="card-body no-results">
      ...
      <button id="reset-search" class="btn btn-warning">Clear Search</button>
    </div>
  </div>
  ...
  <div id="items-container" class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
    ...
    <div class="col item-card" data-name="{{ strtolower($item->item_name) }}"
      data-location="{{ strtolower($item->location_found) }}" data-category="{{ strtolower($item->category ?? '') }}"
      data-status="{{ $item->status }}">
      <div class="card h-100 shadow-sm">
        @if($item->photo)
        <img src="{{ asset('storage/' . $item->photo) }}" class="card-img-top" alt="{{ $item->item_name }}"
        ...
        <div class="card-body d-flex flex-column">
          ...
          <div class="mt-auto d-flex justify-content-between align-items-center">
            <a href="{{ route('found.show', $item->id) }}" class="btn btn-sm btn-outline-warning">
              <i class="fas fa-eye me-1"></i>View Details
            </a>
            <span class="badge rounded-pill {{ $item->status == 'found' ? 'badge-found' : 'badge-returned' }}">
              {{ ucfirst($item->status) }}
            </span>
          </div>
        </div>
